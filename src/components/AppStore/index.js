...
class AppStore extends Component {
  ...
  render() {
    const {searchInput, filteredAppList, activeTabId} = this.state
    const searchResultList = filteredAppList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInput.toLocaleLowerCase()),
    )
    const filteredCategoryList = searchResultList.filter(
      eachApp => eachApp.category === activeTabId,
    )
    return (
      <div className='appStore-app-container'>
        <h1 className='appStore-main-heading'>App Store</h1>
        ...
        <ul className='apps-list-container'>
          {filteredCategoryList.map(appDetails => (
